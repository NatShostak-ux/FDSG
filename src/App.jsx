...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        // Imposta sempre lo Scenario 1 (Phase 1) come default attivo
        if (loadedScenarios.length > 0) {
            setActiveScenarioId(loadedScenarios[loadedScenarios.length - 1].id);
        }
      }
      setLoading(false);
    });
  ...
  const handleSelectPhase = (phaseId) => {
    ...
    // Indirizza sempre allo Scenario 1
    const targetScenarioId = scenarios[scenarios.length - 1]?.id;
  ...
  // Identifica lo scenario visibile (Scenario 1)
  const activeScenario = useMemo(() => {
      return scenarios[scenarios.length - 1] || { title: '', description: '', data: {} };
  }, [scenarios]);

  // RICERCA: Cerca solo nello Scenario 1
  const searchResults = useMemo(() => {
      if (!searchQuery || searchQuery.length < 2 || !activeScenario) return [];
      ...
      Object.entries(activeScenario.data || {}).forEach(([areaId, areaData]) => {
            ...
            <div className="animate-fadeIn">
                <div className="mb-8">
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm relative group">
                        ...
                        <div className="flex items-center justify-between gap-4 mb-4 border-b border-gray-100 pb-4 relative z-50">
                            ...
                            <div className="relative flex-grow max-w-md">
                                <div className={`flex items-center bg-slate-50 rounded-lg transition-all border ${isSearchFocused ? 'ring-2 ring-blue-100 border-blue-300 bg-white' : 'border-gray-200'}`}>
                                    ...
                                    <input 
                                        ...
                                        placeholder="Cerca in questo scenario..."
                    ...
                    <div className="flex-grow min-w-0 transition-all duration-300">
                        {activeView === 'dashboard' ? (
                            ...
                            <AreaEditor
                                ...
                                onFocusHandled={() => setSearchFocusItem(null)}
                            />
                        )}
                    </div>
                </div>
