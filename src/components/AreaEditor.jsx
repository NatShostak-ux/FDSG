    ...
    const restoreBlock = (block) => {
        ...
        setTimeout(() => {
            ...
            if (newElement) {
                ...
                newElement.style.boxShadow = '0 0 0 6px rgba(34, 197, 94, 0.4)'; 
                ...
                <div className="fixed bottom-8 right-8 z-[100]" ref={blockButtonRef}>
                    <div className="relative">
                        <button 
                            ...
                            title="Aggiungi o ripristina un modulo"
                        >
                        ...
                        {blockMenuOpen && (
                            <div className="absolute bottom-full mb-4 right-0 w-80 bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden animate-fadeInFast p-2 z-[61]" ref={blockMenuRef}>
                                ...
                                {/* --- SEZIONE CESTINO (Recupero Moduli) --- */}
                                {orphanedBlocks.length > 0 && (
                                    <>
                                        <div className="h-px bg-gray-100 my-2 mx-2"></div>
                                        <div className="text-[10px] font-bold text-red-400 uppercase tracking-widest p-3 px-4 flex items-center gap-1.5 Outfit">
                                            <RotateCcw size={12}/> Cestino (Moduli Nascosti)
                                        </div>
                                        {orphanedBlocks.map(ob => (
                                            <button key={ob.id} onClick={() => restoreBlock(ob)} className="w-full text-left px-4 py-3 text-sm hover:bg-red-50 hover:text-red-700 flex items-center gap-3.5 rounded-2xl transition-colors font-medium Outfit text-gray-700">
                                                <Plus size={16} className="text-red-400"/> {ob.title}
                                            </button>
                                        ))}
                                    </>
                                )}
