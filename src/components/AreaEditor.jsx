...
const AreaEditor = ({ activeView, activeScenario, updateAreaData, updateProject, updateProjectBatch, updateKSM, isEditor = false, isGeneratingPDF = false }) => {
    ...
    return (
        <div className={`space-y-6 animate-fadeIn relative ${isGeneratingPDF ? 'bg-white p-4' : ''}`}>
            
            {/* Note - visibili solo a schermo */}
            {!isGeneratingPDF && isNotesOpen && (
            ...
            {/* Header Area */}
            <div className={`bg-white rounded-xl p-6 ${isGeneratingPDF ? 'border-b-2 border-gray-800 rounded-none px-0 mb-8 shadow-none' : 'shadow-sm border border-gray-200'}`}>
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    ...
                    <div className="flex items-stretch gap-4 w-full md:w-auto relative">
                        {/* Ruolo Strategico */}
                        <div className={`rounded-xl p-3 flex flex-col justify-center ${isGeneratingPDF ? 'bg-transparent text-gray-800 p-0' : 'shadow-lg bg-slate-900 min-w-[190px]'}`}>
                            <div className="flex items-center justify-between mb-1">
                                <span className={`text-[10px] uppercase font-bold tracking-wider ${isGeneratingPDF ? 'text-gray-500' : 'text-yellow-500'}`}>Ruolo Strategico</span>
                                {!isGeneratingPDF && <button onClick={() => setIsLegendOpen(!isLegendOpen)} className="text-gray-400 hover:text-white"><Info size={14} /></button>}
                            </div>
                            <select value={currentRole.value} onChange={(e) => updateAreaData(activeView, 'importance', parseInt(e.target.value))} disabled={!isEditor || isGeneratingPDF} className={`w-full appearance-none bg-transparent border-0 p-0 py-1 font-semibold text-[15px] ${isGeneratingPDF ? 'text-black text-lg cursor-default' : 'text-white border-b-2 border-yellow-500 cursor-pointer focus:ring-0'}`}>
                                {STRATEGIC_ROLES.map(role => <option key={role.id} value={role.value}>{role.icon} {role.label}</option>)}
                            </select>

                            {/* TOOLTIP POPOVER (visibile solo se non si esporta il pdf e si clicca sulla i) */}
                            {!isGeneratingPDF && isLegendOpen && (
                                <div>
                        ...
                        <div className={`bg-gray-50 border border-gray-200 rounded-xl p-3 flex flex-col justify-center min-w-[180px] ${isGeneratingPDF ? 'bg-transparent border-none p-0' : ''}`}>
            ...
            <Card title="Obiettivi Macro" icon={Target}>
                <AdvancedEditor value={data.objectives || ''} onChange={(val) => updateAreaData(activeView, 'objectives', val)} placeholder={`Definisci gli obiettivi...`} disabled={!isEditor || isGeneratingPDF} />
            </Card>

            <Card title="Evoluzione Temporale (Roadmap 3 Anni)" icon={Calendar}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
                    {[1, 2, 3].map(year => (
                        <div key={year} className={`bg-gray-50 p-3 rounded-lg flex flex-col h-full ${isGeneratingPDF ? '' : 'border border-gray-100'}`}>
                            <div className="text-xs font-bold text-gray-500 uppercase mb-2 text-[10px] tracking-wider">Anno {year}</div>
                            <div className={`relative flex-grow h-full bg-white rounded ${isGeneratingPDF ? '' : 'border border-gray-100'}`}>
                                <AdvancedEditor value={data[`evolution_y${year}`] || ''} onChange={(val) => updateAreaData(activeView, `evolution_y${year}`, val)} placeholder={`Focus Anno ${year}...`} disabled={!isEditor || isGeneratingPDF} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <Card title="Pianificazione Iniziative" icon={Calendar} action={isEditor && !isGeneratingPDF && <Button variant="ghost" icon={Plus} onClick={addProject} className="text-red-700">Aggiungi Progetto</Button>} noPadding>
                                ...
                                <div className="animate-fadeIn">
                                    ...
                                    <div className="space-y-8">
                                        ...
                                        <div className="space-y-4">
                                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Descrizione</label>
                                        ...
                                        <div className="space-y-4 pt-6">
                                            <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Key Enablers</label>
                ...
                {/* Visualizzazione PDF: Elenco completo di tutti i progetti espansi */}
                {isGeneratingPDF && (
                    <div className="p-6 space-y-10 html2pdf__page-break">
                        {areaProjects.length === 0 ? <p className="text-gray-500 italic">Nessun progetto pianificato.</p> : areaProjects.map(p => (
                            <div key={p.id} className="border-b border-gray-200 pb-8 last:border-0">
                                <h3 className="text-xl font-bold mb-3" style={{ color: area.hex }}>{p.title || 'Iniziativa senza nome'}</h3>
                                <div className="flex gap-8 mb-4 text-sm bg-gray-50 p-4 rounded-lg border border-gray-100">
                                    ...
                                    <div><span className="font-bold text-gray-500 uppercase text-[10px] block">Priorità</span> <span style={{ color: area.hex, fontWeight: 'bold'}}>{p.impact}/10</span></div>
                                ...
                                <div className="mb-4">
                                    ...
                                    <AdvancedEditor value={p.description || ''} disabled={true} />
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <ul className="list-none space-y-2 text-sm text-gray-700 font-medium">
                                            {p.enablers.filter(e => e.trim() !== "").map((e, idx) => (
                                                <li key={idx} className="flex items-start gap-2"><span className="text-gray-400 mt-0.5">•</span> {e}</li>
                                            ))}
                                        </ul>
                                    </div>
                                )}
            ...
            <Card title="Key Success Metrics (KSM)" icon={Target} action={isEditor && !isGeneratingPDF && <Button variant="ghost" icon={Plus} onClick={addKSM} className="text-red-700">Aggiungi Metrica</Button>} noPadding>
                    ...
                    {(!Array.isArray(data.ksms) || data.ksms.length === 0) ? (
                        ...
                        data.ksms.map((ksm) => (
                            <div key={ksm.id} className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm relative group space-y-6">
                                {isEditor && !isGeneratingPDF && <button onClick={() => removeKSM(ksm.id)} className="absolute top-6 right-6 text-gray-300 hover:text-red-600"><Trash2 size={18} /></button>}
                                <div>
                                    <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Denominazione Metrica</label>
                                    <input type="text" value={ksm.name || ''} onChange={(e) => updateKSM(activeView, ksm.id, 'name', e.target.value)} disabled={!isEditor || isGeneratingPDF} className="w-full border-0 bg-transparent text-xl font-bold text-gray-800 p-0" placeholder="Nome Metrica" />
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                                    <div><label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Valore As Is (Attuale)</label><input type="text" value={ksm.valueAsIs || ''} onChange={(e) => updateKSM(activeView, ksm.id, 'valueAsIs', e.target.value)} disabled={!isEditor || isGeneratingPDF} className="w-full border-0 bg-transparent text-sm p-0" placeholder="Es. 1.2%" /></div>
                                    <div><label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Target di Massima</label><input type="text" value={ksm.targetValue || ''} onChange={(e) => updateKSM(activeView, ksm.id, 'targetValue', e.target.value)} disabled={!isEditor || isGeneratingPDF} className="w-full border-0 bg-transparent text-sm p-0 text-blue-600 font-bold" placeholder="Es. > 2.5%" /></div>
                                </div>
                                <div><label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Descrizione (Cosa Misura)</label><AdvancedEditor value={ksm.description || ''} onChange={(val) => updateKSM(activeView, ksm.id, 'description', val)} disabled={!isEditor || isGeneratingPDF} /></div>
                            </div>
                        ))
                    )}
                </div>
                        ...
                        <div className="space-y-2 mt-4">
                            {routineTasks.map((task) => (
                                <div key={task.id} className="group flex items-center justify-between p-3 bg-gray-50 border border-gray-100 rounded-xl">
                                    <div className="flex items-center gap-4 flex-grow cursor-pointer" onClick={() => !isGeneratingPDF && toggleRoutineTask(task.id)}>
                                        {!isGeneratingPDF && <button disabled={!isEditor} className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${task.completed ? 'bg-blue-500 border-blue-500 text-white' : 'bg-white'}`}>{task.completed && <Check size={12} strokeWidth={3} />}</button>}
                                        <span className={`text-sm select-none ${task.completed && !isGeneratingPDF ? 'text-gray-400 line-through' : 'text-gray-800 font-medium'}`}><span className="hidden print:inline-block mr-2">•</span>{task.text}</span>
                                    </div>
                                    {isEditor && !isGeneratingPDF && <button onClick={() => removeRoutineTask(task.id)} className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 p-1"><X size={16} /></button>}
                                </div>
                            ))}
                        </div>
                    )}
